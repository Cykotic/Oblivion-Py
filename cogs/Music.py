...
class Player(wavelink.Player):
    ...
    async def choose_track(self, ctx, tracks):
        ...
        embed3.set_author(name="Query Results")
        embed3.set_footer(text=f"Invoked by {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def on_node_ready(self, node):
        print(f"Wavelink Loaded: [{node.identifier}]")
    ...
    async def connect_command(self, ctx, *, channel: t.Optional[discord.VoiceChannel]):
        ...
        embed4 = discord.Embed(
            title = f"Connected to `{channel.name}`", color=0xff7700, delete_after=10)
        await ctx.message.delete()
        await ctx.send(embed = embed4, delete_after=10)
    ...
    async def next_command(self, ctx):
        ...
        embed8 = discord.Embed(
            title = "Playing next track in queue.",
            color=0xff7700,
        )
        await ctx.send(embed=embed8)

    #next_command_error handling
    @next_command.error
    ...
    async def previous_command(self, ctx):
        ...
        embed9 = discord.Embed(
            title = "Playing previous track in queue.",
            color=0xff7700,
        )
    ...
    #shuffle_command_error handling
    ...
    async def queue_command(self, ctx, show: t.Optional[int] = 10):
        ...
        embed12.set_author(name="Query Results")
        embed12.set_footer(text=f"Requested by {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
        embed12.add_field(
    ...
    async def volume_command(self, ctx, volume: int):
        ...
        await ctx.send(f"Volume set at {volume}%.")
    ...
    async def volume_command_error(self, ctx, exc):
        if isinstance(exc, VolumeTooLow):
            ...
            await ctx.send("The volume must not be above 150.")

    @volume_command.command(name="up")
    ...
    @volume_command.command(name="down")
